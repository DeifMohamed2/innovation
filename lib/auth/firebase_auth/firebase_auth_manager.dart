  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      if (userCredential?.user != null) {
        ...
        // If this is a new user, save complete profile to Realtime Database
        // Otherwise, just update the last login timestamp
        final userPath = 'users/${userCredential.user!.uid}';
        final snapshot =
            await userService.checkUserExists(userCredential.user!.uid);
        if (!snapshot) {
          await userService.saveUserToDatabase(userCredential.user!);
        } else {
      ...
      return userCredential == null
          ? null
          : RekoFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
